/*
 *  Copyright (C) 2002 - 2013 Tomasz Kojm <tkojm@quikav.net>
...
test_database_wrap (const char *file, const char *newdb, int bytecode)
{
    ...
    default:
        ...
        while (!strncmp (firstline, "LibQuikAV Warning:", 18));
                ...
                || cli_strbcasestr (dent->d_name, ".cvd"))
            {
                ...
                if ((ret = cl_cvdverify (fname)))
                {
                    ...
                    if (rename (fname, broken))
                    {
                        if (unlink (fname))
                            mprintf
                                ("!Can't remove broken database file %s, please delete it manually and restart freshquik\n",
                                 fname);
                    }
                    else
                    {
          ...
          unsigned int attempt)
{
            ...
            *signo += current->sigs;
            ...
            if (mirror_stats && strlen (ip))
            {
                snprintf (squery, sizeof (squery),
                          "%s.%u.%u.%u.%u.%s.ping.quikav.net", dbname,
    ...
    if (!nodb && (current->version >= newver))
    {
        ...
        if (!outdated && flevel < current->fl)
        {
            ...
            logg ("^Current functionality level = %d, recommended = %d\n",
                  flevel, current->fl);
            logg ("Please check if QuikAV tools are linked against the proper version of libquikav\n");
            logg ("DON'T PANIC! Read http://www.quikav.net/doc/install.html\n");
        }
    ...
    if (nodb)
    {
        ...
        if (ret)
        {
            ...
            {
                snprintf (squery, sizeof (squery),
                          "%s.%u.%u.%u.%u.%s.ping.quikav.net", dbname, 0,
    ...
    {
        logg ("^Your QuikAV installation is OUTDATED!\n");
        logg ("^Current functionality level = %d, recommended = %d\n", flevel,
              current->fl);
        logg ("DON'T PANIC! Read http://www.quikav.net/documents/upgrading-quikav\n");
                 ...
                 unsigned int attempt)
{
    ...
    time (&currtime);
    logg ("QuikAV update process started at %s", ctime (&currtime));
...
#ifdef BUILD_QUIKD
        if ((opt = optget (opts, "NotifyQuikd"))->active)
            notify (opt->strarg);
#endif
