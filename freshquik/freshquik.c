...
help (void)
{
    ...
    mprintf ("\n");
    mprintf ("                   Quik AntiVirus: freshquik  %s\n",
             get_version ());
...
#ifdef BUILD_QUIKD
    mprintf
        ("    --daemon-notify[=/path/quikd.conf]   send RELOAD command to quikd\n");
#endif
    mprintf
        ...
        while (opt)
        {
                ...
                || ret == FCE_FAILEDGET || ret == FCE_MIRRORNOTSYNC)
            {
                ...
                else
                {
                    ...
                    if (!opt)
                    {
                        logg ("Update failed. Your network may be down or none of the mirrors listed in %s is working. Check http://www.quikav.net/doc/mirrors-faq.html for possible reasons.\n", cfgfile);
                    }
                }
...
main (int argc, char **argv)
{
    ...
    {
        mprintf ("!Can't initialize libquikav: %s\n", cl_strerror (retcl));
        return FCE_INIT;
    }

    if ((opts =
         optparse (NULL, argc, argv, 1, OPT_FRESHQUIK, 0, NULL)) == NULL)
    {
    ...
    pt = strdup (cfgfile);
    if ((opts =
         optparse (cfgfile, 0, NULL, 1, OPT_FRESHQUIK, 0, opts)) == NULL)
    {
...
#ifdef HAVE_PWD_H
    /* freshquik shouldn't work with root privileges */
    ...
    if ((opt = optget (opts, "PrivateMirror"))->enabled)
    {
        ...
        do
        {
            if (cli_strbcasestr (opt->strarg, ".quikav.net"))
            {
                logg ("!PrivateMirror: *.quikav.net is not allowed in this mode\n");
                optfree (opts);
    ...
    *updtmpdir = 0;
    ...
    if (optget (opts, "daemon")->enabled)
    {
        ...
        logg ("#freshquik daemon %s (OS: " TARGET_OS_TYPE ", ARCH: "
