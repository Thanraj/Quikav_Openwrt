 ...
 *  Copyright (C) 2002 - 2013 Tringapps, Inc.
 *
...
int
quikd_connect (const char *cfgfile, const char *option)
{
#ifndef	_WIN32
    ...
    if ((opts = optparse (cfgfile, 0, NULL, 1, OPT_QUIKD, 0, NULL)) == NULL)
    {
    ...
    if ((opt = optget (opts, "LocalSocket"))->enabled)
    {
        ...
        {
            logg ("^Quikd was NOT notified: Can't create socket endpoint for %s: %s\n",
        ...
        {
            logg ("^Quikd was NOT notified: Can't connect to quikd through %s: %s\n",
                opt->strarg, strerror(errno));
            closesocket (sockd);
            optfree (opts);
            return -11;
    ...
    if ((opt = optget (opts, "TCPSocket"))->enabled)
    {
        ...
        while (opt) {
            ...
            for (p = res; p != NULL; p = p->ai_next) {
                ...
                {
                    logg ("!%s: Can't connect to quikd on %s:%s: %s\n", option,
...
notify (const char *cfgfile)
{
    ...
    if ((sockd = quikd_connect (cfgfile, "NotifyQuikd")) < 0)
        return 1;

    if (sendln (sockd, "RELOAD", 7) < 0)
    {
        logg ("!NotifyQuikd: Could not write to quikd socket: %s\n", strerror(errno));
        closesocket (sockd);
        return 1;
    }
    ...
    if ((bread = recv (sockd, buff, sizeof (buff), 0)) > 0)
    {
        ...
        {
            logg ("!NotifyQuikd: Unknown answer from quikd: '%s'\n", buff);
            closesocket (sockd);
            return -1;
        }
    }
    ...
    closesocket (sockd);
    logg ("Quikd successfully notified about the update.\n");
    return 0;
}
#endif
