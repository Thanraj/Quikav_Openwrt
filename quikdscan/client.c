 ...
 * Returns 0 if quikd is local, non zero if quikd is remote */
static int isremote(const struct optstruct *opts) {
...
#ifndef _WIN32
    if((opt = optget(quikdopts, "LocalSocket"))->enabled) {
...
#endif
    if(!(opt = optget(quikdopts, "TCPSocket"))->enabled)
        return 0;
    ...
    while (opt) {
        ...
        if ((res = getaddrinfo(ipaddr, port, &hints, &info))) {
            logg("!Can't lookup quikd hostname: %s\n", gai_strerror(res));
            opt = opt->nextarg;
            continue;
        }
        ...
        for (p = info; p != NULL; p = p->ai_next) {
            ...
            if (ret) {
                if (errno == EADDRINUSE) {
                     ...
                     * ours or that quikd is already listening on.
                     */
                    closesocket(s);
                    freeaddrinfo(info);
                    return 0;
...
int get_quikd_version(const struct optstruct *opts)
{
	char *buff;
	int len, sockd;
	struct RCVLN rcv;
...
int reload_quikd_database(const struct optstruct *opts)
{
	char *buff;
	int len, sockd;
	struct RCVLN rcv;
	...
	logg("!Quikd did not reload the database\n");
	closesocket(sockd);
	return 2;
    }
    closesocket(sockd);
...
int client(const struct optstruct *opts, int *infected, int *err)
{
    ...
    if(!remote && optget(quikdopts, "LocalSocket")->enabled && (optget(opts, "fdpass")->enabled || scandash)) {
	scantype = FILDES;
    ...
    maxrec = optget(quikdopts, "MaxDirectoryRecursion")->numarg;
    maxstream = optget(quikdopts, "StreamMaxLength")->numarg;
    if (optget(quikdopts, "FollowDirectorySymlinks")->enabled)
	flags |= CLI_FTW_FOLLOW_DIR_SYMLINK;
    if (optget(quikdopts, "FollowFileSymlinks")->enabled)
