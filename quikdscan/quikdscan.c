...
int main(int argc, char **argv)
{
    ...
    if((opts = optparse(NULL, argc, argv, 1, OPT_QUIKDSCAN, OPT_QUIKSCAN, NULL)) == NULL) {
	mprintf("!Can't parse command line options\n");
	return 2;
    }

    if((quikdopts = optparse(optget(opts, "config-file")->strarg, 0, NULL, 1, OPT_QUIKD, 0, NULL)) == NULL) {
	logg("!Can't parse quikd configuration file %s\n", optget(opts, "config-file")->strarg);
	return 2;
    }
	...
	ret = reload_quikd_database(opts);
	optfree(opts);
	optfree(quikdopts);
	logg_close();
	exit(ret);
    ...
    /* TODO: Implement STATUS in quikd */
...
void help(void)
{
    ...
    mprintf("    --reload                           Request quikd to reload virus database\n");
    mprintf("    --fdpass                           Pass filedescriptor to quikd (useful if quikd is running as a different user)\n");
    mprintf("    --stream                           Force streaming files to quikd (for debugging and unit testing)\n");
    mprintf("\n");
