...
int printinfected;
extern struct optstruct *quikdopts;
#ifndef _WIN32
...
int dconnect() {
...
#ifndef _WIN32
    opt = optget(quikdopts, "LocalSocket");
    if (opt->enabled) {
    ...
    snprintf(port, sizeof(port), "%lld", optget(quikdopts, "TCPSocket")->numarg);

    opt = optget(quikdopts, "TCPAddr");
    while (opt) {
        ipaddr = NULL;
        if (opt->strarg)
        ...
        for (p = info; p != NULL; p = p->ai_next) {
            ...
            if(connect(sockd, p->ai_addr, p->ai_addrlen) < 0) {
                logg("!Could not connect to quikd on %s: %s\n", opt->strarg, strerror(errno));
                closesocket(sockd);
                continue;
            }
...
/* Issues an INSTREAM command to quikd and streams the given file
...
/* Issues a FILDES command and pass a FD to quikd
 * Returns >0 on success, 0 soft fail, -1 hard fail */
...
static int chkpath(const char *path)
{
   ...
   if((opt = optget(quikdopts, "ExcludePath"))->enabled) {
	while(opt) {
...
/* Sends a proper scan request to quikd and parses its replies
...
int dsresult(int sockd, int scantype, const char *filename, int *printok, int *errors) {
	...
	*printok = 0;
	...
	if(len > 7) {
		...
		*colon = 0;
		...
		if (!strncmp(bol, unkco, sizeof(unkco) - 1))
		    logg("quikd replied \"UNKNOWN COMMAND\". Command was %s\n", 
    ...
    if(!beenthere) {
        if (!filename) {
	    logg("STDIN: noreply from quikd\n.");
	    return -1;
	}
        if(QUIKSTAT(filename, &sb) == -1) {
	    logg("~%s: stat() failed with %s, quikd may not be responding\n",
		 filename, strerror(errno));
	    return -1;
	}
	if(!S_ISDIR(sb.st_mode)) {
	    logg("~%s: no reply from quikd\n", filename);
	    return -1;
	}
    }
    return infected;
...
static int dspresult(struct client_parallel_data *c) {
	...
	if(!id) {
	    logg("!Bogus session id from quikd\n");
	    return 1;
	}
	filename = (*id)->file;
	if(len > 7) {
	...
	*id = (*id)->next;
	free(bol);
    } while(rcv.cur != rcv.buf); /* quikd sends whole lines, so, on partial lines, we just assume
...
static int parallel_callback(STATBUF *sb, char *filename, const char *path, enum cli_ftw_reason reason, struct cli_ftw_cbdata *data) {
    ...
    while(1) {
	/* consume all the available input to let some of the quikd
...
int parallel_client_scan(char *file, int scantype, int *infected, int *err, int maxlevel, int flags) {
    ...
    *err += cdata.errors;
    ...
    if(cdata.ids) {
	logg("!Quikd closed the connection before scanning all files.\n");
	return 1;
    }
    if(cdata.errors)
	return 1;
