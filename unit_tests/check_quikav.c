...
#include "../libquikav/dsig.h"
#include "../libquikav/fpu.h"
#include "checks.h"

static int fpu_words  = FPU_ENDIAN_INITME;
#define NO_FPU_ENDIAN (fpu_words == FPU_ENDIAN_UNKNOWN)
#define EA06_SCAN strstr(file, "quik.ea06.exe")
...
START_TEST (test_cl_scandesc)
{
    ...
    if (!FALSE_NEGATIVE) {
      ...
      fail_unless_fmt(virname && !strcmp(virname, "QuikAV-Test-File.UNOFFICIAL"), "virusname: %s", virname);
    }
    close(fd);
}
END_TEST
...
static void init_testfiles(void)
{
	...
	if (strncmp(dirent->d_name, "quik", 4))
	    continue;
        i++;
...
{
    unsigned int sigs = 0;
    const char *hdb = OBJDIR"/quikav.hdb";
...
START_TEST (test_cl_scanmap_callback_mem)
{
    ...
    if (!FALSE_NEGATIVE) {
      fail_unless_fmt(ret == CL_VIRUS, "cl_scanmap_callback failed for %s: %s", file, cl_strerror(ret));
      fail_unless_fmt(virname && !strcmp(virname, "QuikAV-Test-File.UNOFFICIAL"), "virusname: %s for %s", virname, file);
    }
    close(fd);
    cl_fmap_close(map);
