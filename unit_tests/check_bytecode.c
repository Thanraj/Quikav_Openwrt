...
#include "../libquikav/bytecode.h"
#include "checks.h"
#include "../libquikav/dconf.h"
#include "../libquikav/bytecode_priv.h"
		    ...
		    int testmode)
{
    ...
    if (have_quikjit && !nojit && nojit != -1 && !testmode) {
...
START_TEST (test_matchwithread_jit)
{
    ...
    sect.uraw = 1;
    sect.ursz = 512;
    runtest("input/matchwithread.cbc", 0, 0, 0, "../test/quik.exe", &pedata,
	    &sect, "QuikAV-Test-File-detected-via-bytecode", 0);
}
END_TEST
...
START_TEST (test_matchwithread_int)
{
    ...
    sect.uraw = 1;
    sect.ursz = 512;
    runtest("input/matchwithread.cbc", 0, 0, 1, "../test/quik.exe", &pedata,
	    &sect, "QuikAV-Test-File-detected-via-bytecode", 0);
}
END_TEST
