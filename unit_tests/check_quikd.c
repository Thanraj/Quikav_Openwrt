...
#include "libquikav/cltypes.h"

#ifdef CHECK_HAVE_LOOPS

static int sockd;
#define SOCKET "quikd-test.socket"
...
#define SCANFILE BUILDDIR"/../test/quik.exe"
#define FOUNDREPLY SCANFILE": QuikAV-Test-File.UNOFFICIAL FOUND"
...
#define VERSION_REPLY "QuikAV "REPO_VERSION""VERSION_SUFFIX
...
enum idsession_support {
    IDS_OK, /* accepted */
    IDS_REJECT,
    /* after sending this message, quikd will reply,  then accept 
...
#define EXPECT_INSTREAM "stream: QuikAV-Test-File.UNOFFICIAL FOUND\n"
#define EXPECT_INSTREAM0 "stream: QuikAV-Test-File.UNOFFICIAL FOUND"
...
static size_t prepare_instream(char *buf, size_t off, size_t buflen)
{
    ...
    int fd, nread;
    uint32_t chunk;
    fail_unless_fmt(QUIKSTAT(SCANFILE, &stbuf) != -1, "stat failed for %s: %s", SCANFILE, strerror(errno));
...
#define FOUNDFDREPLY " QuikAV-Test-File.UNOFFICIAL FOUND"
...
START_TEST (test_connections)
{
	...
	free(sock);
	/* now see if quikd is able to do anything else */
	for (i=0;i<10;i++) {
	    conn_setup();
...
START_TEST (test_stream)
{
    ...
    recvdata = recvfull(sockd, &len);
    fail_unless_fmt(!strcmp(recvdata,"stream: QuikAV-Test-File.UNOFFICIAL FOUND"),
		    "Wrong reply: %s\n", recvdata);
    free(recvdata);
...
static Suite *test_quikd_suite(void)
{
    Suite *s = suite_create("quikd");
    TCase *tc_commands, *tc_stress;
    tc_commands = tcase_create("quikd commands");
    ...
    tc_stress = tcase_create("quikd stress test");
...
{
    int nf;
    Suite *s = test_quikd_suite();
    SRunner *sr = srunner_create(s);
    srunner_set_log(sr, BUILDDIR"/test-quikd.log");
...
int main(void)
{
    puts("\n*** Check version too old, quikd tests not run!\n");
