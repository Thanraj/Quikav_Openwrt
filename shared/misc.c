...
/* CL_NOLIBQUIKAV means to omit functions that depends on libquikav */
#ifndef CL_NOLIBQUIKAV
char *freshdbdir(void)
{
    ...
    dbdir = cl_retdbdir();
    if((opts = optparse(CONFDIR_FRESHQUIK, 0, NULL, 0, OPT_FRESHQUIK, 0, NULL))) {
...
void print_version(const char *dbdir)
{
    ...
    if(db_version) {
	printf("QuikAV %s/%u/%s", get_version(), db_version, ctime(&db_time));
    } else {
	printf("QuikAV %s\n",get_version());
    }
...
int check_flevel(void)
{
	...
	fprintf(stderr, "ERROR: This tool requires libquikav with functionality level %u or higher (current f-level: %u)\n", CL_FLEVEL, cl_retflevel());
	return 1;
    }
    return 0;
}
