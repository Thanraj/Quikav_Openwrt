...
#include "libquikunrar/unrarhlp.h"
...
void *rar_malloc(size_t size)
{
	...
	rar_dbgmsg("UNRAR: rar_malloc(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", size);
	return NULL;
    }
...
void *rar_realloc2(void *ptr, size_t size)
{
    ...
    if(!size || size > RAR_MAX_ALLOCATION) {
	rar_dbgmsg("UNRAR: rar_realloc2(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", size);
	return NULL;
    }
