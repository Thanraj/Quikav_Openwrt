...
#include "libquikav/bytecode_detect.h"
#include "target.h"
#include "fpu.h"

#ifndef _WIN32
extern const struct quik_option *quik_options;
#else
__declspec(dllimport) extern const struct quik_option *quik_options;
#endif
...
static struct _cfgfile {
    ...
    { "quikd.conf",	    OPT_QUIKD	    },
    { "freshquik.conf",	    OPT_FRESHQUIK   },
    { "quikav-milter.conf", OPT_MILTER	    },
    { NULL,		    0		    }
};
...
static void printopts(struct optstruct *opts, int nondef)
{
	...
	if(quik_options[opts->idx].owner & OPT_DEPRECATED) {
	    if(opts->active)
		printf("*** %s is DEPRECATED ***\n", opts->name);
	    opts = opts->next;
	    continue;
	}
	if(nondef && (opts->numarg == quik_options[opts->idx].numarg) && ((opts->strarg == quik_options[opts->idx].strarg) || (opts->strarg && quik_options[opts->idx].strarg && !strcmp(opts->strarg, quik_options[opts->idx].strarg)))) {
	    opts = opts->next;
	    continue;
	}
	if(!opts->enabled) 
	    printf("%s disabled\n", opts->name);
	else switch(quik_options[opts->idx].argtype) {
...
static int printconf(const char *name)
{
    ...
    printf("##\n## %s - automatically generated by quikconf "VERSION"\n##\n", name);
    printf("\n# Comment out or remove the line below.\nExample\n");

    for(i = 0; quik_options[i].owner; i++) {
	cpt = &quik_options[i];
...
{
    printf("\n");
    printf("           Quik AntiVirus: Configuration Tool %s\n", get_version());
...
{
	const char *dir;
	char path[512], dbdir[512], quikd_dbdir[512], *pt;
	struct optstruct *opts, *toolopts;
	const struct optstruct *opt;
	unsigned int i, j;
	struct cli_environment env;

    opts = optparse(NULL, argc, argv, 1, OPT_QUIKCONF, 0, NULL);
    if(!opts) {
	...
	if(cfgfile[i].tool == OPT_FRESHQUIK) {
	    opt = optget(toolopts, "DatabaseDirectory");
	    strncpy(dbdir, opt->strarg, sizeof(dbdir));
	    dbdir[sizeof(dbdir) - 1] = 0;
	} else if(cfgfile[i].tool == OPT_QUIKD) {
	    opt = optget(toolopts, "DatabaseDirectory");
	    strncpy(quikd_dbdir, opt->strarg, sizeof(quikd_dbdir));
	    quikd_dbdir[sizeof(quikd_dbdir) - 1] = 0;
	}
	optfree(toolopts);
    }
    optfree(opts);
	...
	printf("WARNING: Version mismatch: libquikav=%s, quikconf=%s\n", cl_retver(), get_version());
    ...
    if(strcmp(dbdir, quikd_dbdir))
	printf("WARNING: freshquik.conf and quikd.conf point to different database directories\n");
    print_dbs(dbdir);
