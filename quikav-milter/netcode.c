...
static int nc_connect(int s, struct CP_ENTRY *cpe) {
	...
	if(res < 1) {
	    ...
	    logg("*Failed to establish a connection to quikd\n");
	    close(s);
	    return -1;
	}
...
int nc_send(int s, const void *buff, size_t len) {
	...
	while(1) {
		...
		logg("!Failed to stream to quikd\n");
		close(s);
		return 1;
	    }
	    break;
...
int nc_sendmsg(int s, int fd) {
    ...
    *(int *)CMSG_DATA(cmsg) = fd;
	...
	char er[256];
	strerror_print("!quikfi_eom: FD send failed");
	close(s);
    }
    return ret;
}
...
char *nc_recv(int s) {
	...
	now = time(NULL);
	if(now >= timeout) {
	    logg("!Timed out while reading quikd reply\n");
	    close(s);
	    return NULL;
	}
	tv.tv_sec = timeout - now;
	...
	if(len && buf[len-1] == '\n') break;
	if(len >= sizeof(buf)) {
	    logg("!Overlong reply from quikd\n");
	    close(s);
	    return NULL;
	}
    }
...
int nc_connect_rand(int *main, int *alt, int *local) {
    ...
    *local = (cpe->server->sa_family == AF_UNIX);
	...
	if(nc_send(*main, "nINSTREAM\n", 10)) {
	    logg("!Failed to communicate with quikd\n");
	    close(*main);
	    return 1;
	}
    }
