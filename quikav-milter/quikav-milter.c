...
static void milter_exit(int sig) {
    ...
    logg("*quikav-milter: milter_exit, signal %d\n", sig);
...
int main(int argc, char **argv) {
    ...
    descr.xxfi_name = "QuikAV";			/* filter name */
    descr.xxfi_version = SMFI_VERSION;		/* milter version */
    descr.xxfi_flags = SMFIF_QUARANTINE;	/* flags */
    descr.xxfi_connect = quikfi_connect;	/* connection info filter */
    descr.xxfi_envfrom = quikfi_envfrom;	/* envelope sender filter */
    descr.xxfi_envrcpt = quikfi_envrcpt;	/* envelope recipient filter */
    descr.xxfi_header = quikfi_header;		/* header filter */
    descr.xxfi_body = quikfi_body;		/* body block */
    descr.xxfi_eom = quikfi_eom;		/* end of message */
    descr.xxfi_abort = quikfi_abort;		/* message aborted */
	...
	if (((opt = optget(opts, "ReportHostname"))->enabled &&
	    ...
	    snprintf(xvirushdr, sizeof(xvirushdr), "quikav-milter %s at %s",
		     get_version(), myname);
	} else {
	    snprintf(xvirushdr, sizeof(xvirushdr), "quikav-milter %s",
		     get_version());
	}
	...
	openlog("quikav-milter", LOG_PID, fac);
	logg_syslog = 1;
    }
#endif
