...
static int cpool_addunix(char *path) {
	...
	logg("!Unix quikd socket must be an absolute path\n");
	return 1;
    }
...
- probe dead if (last check > 2 min || no quikd available)
*/
...
static void cpool_probe(void) {
    ...
    if(!cp->alive)
	logg("^No quikd server appears to be available\n");
}
...
void cpool_init(struct optstruct *opts) {
    ...
    if((opt = optget(opts, "QuikdSocket"))->enabled) {
	while(opt) {
		...
		logg("!Failed to parse QuikdSocket directive '%s'\n", socktype);
		failed = 1;
	    }
	    if(failed) break;
	    opt = opt->nextarg;
