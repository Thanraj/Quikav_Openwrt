...
static void nullify(SMFICTX *ctx, struct QUIKFI *cf, enum CFWHAT closewhat) {
...
static sfsistat sendchunk(struct QUIKFI *cf, unsigned char *bodyp, size_t len, SMFICTX *ctx) {
	...
	if((ret = sendchunk(cf, (unsigned char *)"From quikav-milter\n", 19, ctx)) != SMFIS_CONTINUE)
	    return ret;
	cf->totsz -= 1;
    }
	...
	if(len < QUIKFIBUFSZ - cf->bufsz) {
	    memcpy(&cf->buffer[cf->bufsz], bodyp, len);
	    cf->bufsz += len;
	} else if(len < QUIKFIBUFSZ) {
	    memcpy(&cf->buffer[cf->bufsz], bodyp, QUIKFIBUFSZ - cf->bufsz);
	    cf->sendme = htonl(QUIKFIBUFSZ);
	    sendfailed = nc_send(cf->main, &cf->sendme, QUIKFIBUFSZ + 4);
	    len -= (QUIKFIBUFSZ - cf->bufsz);
	    memcpy(cf->buffer, &bodyp[QUIKFIBUFSZ - cf->bufsz], len);
	    cf->bufsz = len;
	} else {
...
sfsistat quikfi_header(SMFICTX *ctx, char *headerf, char *headerv) {
    ...
    if(!(cf = (struct QUIKFI *)smfi_getpriv(ctx)))
...
sfsistat quikfi_connect(_UNUSED_ SMFICTX *ctx, char *hostname, _SOCK_ADDR *hostaddr) {
    while(1) {
...
static sfsistat action_quarantine(SMFICTX *ctx) {
    if(smfi_quarantine(ctx, "quarantined by quikav-milter") != MI_SUCCESS) {
...
static sfsistat action_reject_msg(SMFICTX *ctx) {
    ...
    if(!rejectfmt || !(cf = (struct QUIKFI *)smfi_getpriv(ctx)))
	return SMFIS_REJECT;
...
sfsistat quikfi_envfrom(SMFICTX *ctx, char **argv) {
    ...
    if(!(cf = (struct QUIKFI *)malloc(sizeof(*cf)))) {
	logg("!Failed to allocate QUIKFI struct\n");
	return FailAction;
    }
    cf->totsz = 0;
    cf->bufsz = 0;
