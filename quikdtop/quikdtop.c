...
static void resize(void)
{
	...
	quikd_header = malloc(maxx + 1);
	OOM_CHECK(quikd_header);
	assert(quikd_header && queue_header);
	strncpy(queue_header, global.num_quikd>1 ? CMDHEAD2 : CMDHEAD, maxx);
	strncpy(quikd_header, SUMHEAD, maxx);
	queue_header[maxx] = '\0';
	quikd_header[maxx] = '\0';
	p = queue_header + strlen(queue_header);
	while(p < queue_header+maxx)
		*p++ = ' ';
	p = quikd_header + strlen(quikd_header);
	while(p < quikd_header+maxx)
		*p++ = ' ';
}
...
static void init_windows(int num_quikd)
{
	resize();
	...
	stats_head_window = subwin(stdscr, num_quikd+1, maxx, 1, 0);
	maxystats = maxy-num_quikd-3;
	stats_window = subwin(stdscr, maxystats, maxx, num_quikd+2, 0);
	...
	if (num_quikd > 1) {
		status_bar_keys[3] = "^ - previous quikd";
		status_bar_keys[4] = "v - next quikd";
	}
}

static void init_ncurses(int num_quikd, int use_default)
{
...
static void header(void)
{
	...
	mvwprintw(header_window, 0, 0, "  QuikdTOP version %s   ", get_version());
	time(&t);
...
/*	win_start(version_window, version_color);
	...
	print_colored(version_window, quikd_version ? quikd_version : "Unknown");
...
static void __noreturn exit_program(enum exit_reason reason, const char *func, unsigned line)
{
	switch(reason) {
		...
		case FAIL_INITIAL_CONN:
			exit_reason = "Unable to connect to all quikds";
			break;
		case OUT_OF_MEMORY:
			exit_reason = "Out of memory";
			break;
		case RECONNECT_FAIL:
			exit_reason = "Failed to reconnect to quikd after connection was lost";
			break;
		case SIGINT_REASON:
...
static void output_queue(size_t line, ssize_t max)
{
	...
	for (i=0,j=0;i<global.n;i++) {
		if (detail_selected == -1 || detail_is_selected(tasks[i].quikd_no-1)) {
	...
	for(i=0;i<max;i++) {
		...
		if(cmde) {
			...
			if(filstart) {
				++filstart;
				if (detail_selected == -1 && global.num_quikd > 1)
					mvwprintw(stats_window, line + i, 0, "%2u %s", filtered_tasks[i].quikd_no, cmd + 1);
				else
...
static void parse_queue(conn_t *conn, char* buf, size_t len, unsigned idx)
{
	do {
		...
		global.tasks[global.n-1].quikd_no = idx + 1;
...
static int output_stats(struct stats *stats, unsigned idx)
{
	...
	if (sel && !stats->stats_unsupp) {
		memset(line, ' ', maxx+1);
		snprintf(line, maxx-1, "Details for Quikd version: %s", stats->version);
		line[maxx] = '\0';
		line[strlen(line)]= ' ';
...
static void output_all(void)
{
	...
	mvwprintw(stats_head_window, 0, 0, "%s", quikd_header);
...
{
    printf("\n");
    printf("           Quik AntiVirus: Monitoring Tool %s\n", get_version());
    printf("           By The QuikAV Team: http://www.quikav.net/about.html#credits\n");
    printf("           (C) 2008-2015 Quik AV, Inc.\n\n");
    printf("quikdtop [-hVc] [host[:port] /path/to/quikd.socket ...]\n\n");
    printf("    --help                 -h         Show help\n");
    printf("    --version              -V         Show version\n");
    printf("    --config-file=FILE     -c FILE    Read quikd's configuration files from FILE\n");
    printf("    --defaultcolors	       -d	  Use default terminal colors\n");
    printf("	host[:port]			  Connect to quikd on host at port (default 3310)\n");
    printf("    /path/to/quikd.socket		  Connect to quikd over a local socket\n");
    printf("\n");
    return;
}
...
static void setup_connections(int argc, char *argv[])
{
    ...
    struct optstruct *quikd_opts;
    unsigned i;
    char *conn = NULL;

    opts = optparse(NULL, argc, argv, 1, OPT_QUIKDTOP, 0, NULL);
    if (!opts) {
    ...
    if (!opts->filename || !opts->filename[0]) {
        ...
        const char *quikd_conf = optget(opts, "config-file")->strarg;

        if ((quikd_opts = optparse(quikd_conf, 0, NULL, 1, OPT_QUIKD, 0, NULL)) == NULL) {
            fprintf(stderr, "Can't parse quikd configuration file %s\n", quikd_conf);
            EXIT_PROGRAM(FAIL_CMDLINE);
        }

        if((opt = optget(quikd_opts, "LocalSocket"))->enabled) {
            conn = strdup(opt->strarg);
            if (!conn) {
                fprintf(stderr, "Can't strdup LocalSocket value\n");
                EXIT_PROGRAM(FAIL_INITIAL_CONN);
            }
        } else if ((opt = optget(quikd_opts, "TCPSocket"))->enabled) {
            char buf[512];
            const struct optstruct *opt_addr;
            const char *host = "localhost";
            if ((opt_addr = optget(quikd_opts, "TCPAddr"))->enabled) {
                host = opt_addr->strarg;
            }
            snprintf(buf, sizeof(buf), "%lld", opt->numarg);
            conn = make_ip(host, buf);
        } else {
            fprintf(stderr, "Can't find how to connect to quikd\n");
            EXIT_PROGRAM(FAIL_INITIAL_CONN);
        }
    ...
    global.all_stats = calloc(global.num_quikd, sizeof(*global.all_stats));
    OOM_CHECK(global.all_stats);
    global.conn = calloc(global.num_quikd, sizeof(*global.conn));
...
static int show_help(void)
{
	...
	explain("NO","Unique quikd number");
	...
	explain("HOST", "Which quikd, local means unix socket");
	...
	explain("Pool","Memory usage reported by libquikav's pool");
...
int main(int argc, char *argv[])
{
	...
	init_ncurses(global.num_quikd, default_colors);
	...
	do {
		...
		switch(ch) {
			...
			case 'r':
				...
				break;
			case KEY_UP:
				if (global.num_quikd > 1) {
					if (detail_selected == -1)
						detail_selected = global.num_quikd-1;
					else
						--detail_selected;
				}
				break;
			case KEY_DOWN:
				if (global.num_quikd > 1) {
					if (detail_selected == -1)
						...
						if((unsigned)++detail_selected >= global.num_quikd)
							detail_selected = -1;
					}
				}
				break;
