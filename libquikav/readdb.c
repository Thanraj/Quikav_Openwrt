/*
 *  Copyright (C) 2007-2014 Quik AV, Inc.
 *
 *  Authors: Tomasz Kojm
 *
...
int cli_sigopts_handler(struct cli_matcher *root, const char *virname, const char *hexsig, uint8_t sigopts, uint16_t rtype, uint16_t type, const char *offset, uint8_t target, const uint32_t *lsigid, unsigned int options)
{
    ...
    if (sigopts & ACPATT_OPTION_WIDE) {
        ...
        /* quikav-specific wildcards need to be handled here! */
...
/* FIXME: removing this code because anchored bytes are not sufficiently 
   general for the purposes of yara rule to QuikAV sig conversions.
   1. QuikAV imposes a maximum value for the upper range limit of 32:
      #define AC_CH_MAXDIST 32
      Values larger cause an error in matcher-ac.c
   2. If the upper range values is not present, QuikAV sets the missing
...
*/
    ...
    if (((ovr = strchr(res, '{')) && ((ovr - res) == 2)) ||
        ...
        cli_errmsg("parse_yara_hex_string: Single byte subpatterns unsupported in QuikAV\n");
        free(res);
        if (ret != NULL)
            *ret = CL_EMALFDB;
        return NULL;
...
static int load_oneyara(YR_RULE *rule, int chkpua, struct cl_engine *engine, unsigned int options, unsigned int *sigs)
{
    ...
    */
    ...
    /*** verification step - can quikav load it?       ***/
    ...
    if (str_error > 0) {
        cli_warnmsg("load_oneyara: quikav cannot support %d input strings, skipping %s\n", str_error, newident);
        yara_malform++;
...
#endif
		QUIKSTAT(fname, &dbstat->stattab[dbstat->entries - 1]);
		free(fname);
	    }
	}
    }
