 ...
 *  Copyright (C) 2007-2008 Tringapps, Inc.
 *
 *  Authors: Nigel Horne
 *
 ...
 *
 * If you use the load balancing feature of quikav-milter to run quikd on
...
parseEmailBody(message *messageIn, text *textIn, mbox_ctx *mctx, unsigned int recursion_level)
{
		...
		case MULTIPART:
			...
			if(aText && (textIn == NULL)) {
				if((!infected) && (fb = fileblobCreate()) != NULL) {
					...
					/*fileblobAddData(fb, "Received: by quikd (textpart)\n", 30);*/
		...
		case MESSAGE:
			...
			if((strcasecmp(mimeSubtype, "rfc822") == 0) ||
			   (strcasecmp(mimeSubtype, "delivery-status") == 0)) {
				...
				cli_warnmsg("Unsupported message format `%s' - if you believe this file contains a virus, submit it to www.quikav.net\n", mimeSubtype);
	...
	if(aText && (textIn == NULL)) {
		...
		for(t = aText; t && (rc != VIRUS); t = t->t_next) {
			...
			fileblobAddData(fb, (const unsigned char *)"Received: by quikd (bounce)\n", 28);
			fileblobSetCTX(fb, mctx->ctx);
		   ...
		   ((t_line = bounceBegin(mainMessage)) != NULL))
			...
			if(messageGetMimeType(mainMessage) == MESSAGE)
				...
				if((fb = fileblobCreate()) != NULL) {
					...
					fileblobSetFilename(fb, mctx->dir, "bounce");
					fileblobAddData(fb,
						(const unsigned char *)"Received: by quikd (bounce)\n",
						28);
...
rfc2047(const char *in)
{
	...
	while(*in) {
		...
		if((encoding != 'q') && (encoding != 'b')) {
			cli_warnmsg("Unsupported RFC2047 encoding type '%c' - if you believe this file contains a virus, submit it to www.quikav.net\n", encoding);
			free(out);
			out = NULL;
			break;
		}
...
rfc1341(message *m, const char *dir)
{
	...
	snprintf(pdir, sizeof(pdir) - 1, "%s"PATHSEP"quikav-partial", tmpdir);
 ...
 *	if(feof(fin)) 
	...
	cli_errmsg("Invalid call to getline_from_mbox(). Refer to http://www.quikav.net/doc/install.html\n");
	return NULL;
    }
...
exportBounceMessage(mbox_ctx *mctx, text *start)
{
	 ...
	 * message - quikscan will find that in the
	...
	switch(messageGetMimeType(aMessage)) {
		...
		case MESSAGE:
			...
			messageAddStrAtTop(aMessage,
				"Received: by quikd (message/rfc822)");
#endif
#ifdef	SAVE_TO_DISC
			/*
