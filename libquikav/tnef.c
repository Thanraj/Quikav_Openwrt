			...
			default:
				...
				if(cli_debug_flag) {
					...
					if(fout >= 0) {
						...
						cli_warnmsg("Saving dump to %s:  refer to http://www.quikav.net/doc/install.html\n", filename);
...
tnef_message(fmap_t *map, off_t *pos, uint16_t type, uint16_t tag, int32_t length, off_t fsize)
{
	...
	switch(tag) {
		case attBODY:
			cli_warnmsg("TNEF body not being scanned - if you believe this file contains a virus, submit it to www.quikav.net\n");
			break;
#ifdef	CL_DEBUG
		case attTNEFVERSION:
