...
void *mpool_malloc(struct MP *mp, size_t size) {
  ...
  if (!size || sbits == FRAGSBITS) {
    cli_errmsg("mpool_malloc(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", (unsigned long) size);
    return NULL;
  }
...
void *mpool_realloc(struct MP *mp, void *ptr, size_t size) {
  ...
  if(!size || !(csize = from_bits(f->u.a.sbits))) {
    cli_errmsg("mpool_realloc(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", (unsigned long) size);
    return NULL;
  }
...
char *cli_mpool_strdup(mpool_t *mp, const char *s) {
  ...
  if(s == NULL) {
    cli_errmsg("cli_mpool_strdup(): s == NULL. Please report to http://bugs.quikav.net\n");
    return NULL;
  }
...
char *cli_mpool_virname(mpool_t *mp, const char *virname, unsigned int official) {
  ...
  if(!virname[0]) {
	...
	snprintf(buf, sizeof(buf), "Possibly-Unwanted-Application(www.quikav.net/support/pua).%s", virname + 4);
	buf[sizeof(buf)-1] = '\0';
	virname = buf;
    }
#endif
 ...
 * these symbols are declared in libquikav.map */
void mpool_free() {}
void mpool_create() {}
void mpool_destroy() {}
void mpool_getstats() {}
