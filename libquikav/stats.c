...
#include "libquikav/dconf.h"
#include "libquikav/stats_json.h"
#include "libquikav/stats.h"
#include "libquikav/hostid.h"
#include "libquikav/www.h"
...
void quikav_stats_add_sample(const char *virname, const unsigned char *md5, size_t size, stats_section_t *sections, void *cbdata)
{
    cli_intel_t *intel;
    ...
    if ((intel->engine->cb_stats_get_size))
        ...
        submit = (quikav_stats_get_size(cbdata) >= intel->maxmem);

    if (submit == 0) {
        if ((intel->engine->cb_stats_get_num))
            submit = (intel->engine->cb_stats_get_num(cbdata) >= intel->maxsamples);
        else
            submit = (quikav_stats_get_num(cbdata) >= intel->maxsamples);
    }
    ...
    if (err) {
        cli_warnmsg("quikav_stats_add_sample: locking mutex failed (err: %d): %s\n", err, strerror(err));
        return;
    }
#endif
    ...
    if (err) {
        cli_warnmsg("quikav_stats_add_sample: unlcoking mutex failed (err: %d): %s\n", err, strerror(err));
    }
#endif
    return;
}

void quikav_stats_flush(struct cl_engine *engine, void *cbdata)
{
    cli_intel_t *intel;
    ...
    err = pthread_mutex_lock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_flush: locking mutex failed (err: %d): %s\n", err, strerror(err));
        return;
    }
#endif
    ...
    err = pthread_mutex_unlock(&(intel->mutex));
    if (err)
        cli_warnmsg("quikav_stats_flush: unlocking mutex failed (err: %d): %s\n", err, strerror(err));
#endif
}
...
void quikav_stats_submit(struct cl_engine *engine, void *cbdata)
{
    char *json;
    ...
    err = pthread_mutex_lock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_submit: locking mutex failed (err: %d): %s\n", err, strerror(err));
    ...
    err = pthread_mutex_unlock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_submit: unlocking mutex failed (err: %d): %s\n", err, strerror(err));
    }
#endif
    ...
    if (json) {
        submit_post(STATS_HOST, STATS_PORT, "PUT", "/quikav/1/submit/stats", json, myintel.timeout);
        free(json);
    }
...
void quikav_stats_remove_sample(const char *virname, const unsigned char *md5, size_t size, void *cbdata)
{
    cli_intel_t *intel;
    cli_flagged_sample_t *sample;
    int err;
    ...
    err = pthread_mutex_lock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_remove_sample: locking mutex failed (err: %d): %s\n", err, strerror(err));
        return;
    }
#endif
    ...
    err = pthread_mutex_unlock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_remove_sample: unlocking mutex failed (err: %d): %s\n", err, strerror(err));
    }
#endif
}

void quikav_stats_decrement_count(const char *virname, const unsigned char *md5, size_t size, void *cbdata)
{
    cli_intel_t *intel;
    cli_flagged_sample_t *sample;
    int err;
    ...
    err = pthread_mutex_lock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_decrement_count: locking mutex failed (err: %d): %s\n", err, strerror(err));
        return;
    }
#endif
 ...
 quikav_stats_decrement_end:
#ifdef CL_THREAD_SAFE
    err = pthread_mutex_unlock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_decrement_count: unlocking mutex failed (err: %d): %s\n", err, strerror(err));
    }
#endif
    return;
}

size_t quikav_stats_get_num(void *cbdata)
{
    cli_intel_t *intel;
...
size_t quikav_stats_get_size(void *cbdata)
{
    cli_intel_t *intel;
    cli_flagged_sample_t *sample;
    size_t sz, i;
    ...
    err = pthread_mutex_lock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_get_size: locking mutex failed (err: %d): %s\n", err, strerror(err));
        return sz;
    }
#endif
    ...
    err = pthread_mutex_unlock(&(intel->mutex));
    if (err) {
        cli_warnmsg("quikav_stats_get_size: unlocking mutex failed (err: %d): %s\n", err, strerror(err));
    }
#endif
...
#if defined(_WIN32)
char *quikav_stats_get_hostid(void *cbdata)
{
...
{
    engine->cb_stats_add_sample = quikav_stats_add_sample;
    engine->cb_stats_submit = quikav_stats_submit;
}
