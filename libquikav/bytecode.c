/*
 *  Load, and verify QuikAV bytecode.
 *
...
static int parseApis(struct cli_bc *bc, unsigned char *buffer)
{
	...
	cli_dbgmsg("bytecode using API %u, but highest API known to libquikav is %u, skipping\n", maxapi, cli_apicall_maxapi);
	return CL_BREAK;
    }
...
static int parseGlobals(struct cli_bc *bc, unsigned char *buffer)
{
	...
	cli_dbgmsg("bytecode using global %u, but highest global known to libquikav is %u, skipping\n", maxglobal, cli_apicall_maxglobal);
	return CL_BREAK;
    }
...
static int set_mode(struct cl_engine *engine, enum bytecode_mode mode)
{
	...
	if (mode == CL_BYTECODE_MODE_OFF || have_quikjit) {
...
int cli_bytecode_prepare2(struct cl_engine *engine, struct cli_all_bc *bcs, unsigned dconfmask)
{
    ...
    if (rc != CL_SUCCESS) {
	cli_warnmsg("Bytecode: BC_STARTUP failed to run, disabling ALL bytecodes! Please report to http://bugs.quikav.net\n");
	...
	if ((unsigned int)rc != (unsigned int)0xda7aba5e) {
	    cli_warnmsg("Bytecode: selftest failed with code %08x. Please report to http://bugs.quikav.net\n",
			rc);
	    if (engine->bytecode_mode == CL_BYTECODE_MODE_TEST)
		return CL_EBYTECODE_TESTFAIL;
	}
...
void cli_bytecode_describe(const struct cli_bc *bc)
{
    ...
    switch (bc->kind) {
	case BC_GENERIC:
	    puts("generic, not loadable by quikscan/quikd");
	    break;
	case BC_STARTUP:
    ...
    switch (bc->kind) {
	case BC_GENERIC:
	    puts("N/A (loaded in quikbc only)");
	    break;
	case BC_LOGICAL:
