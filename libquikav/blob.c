...
fileblobDestroy(fileblob *fb)
{
	...
	if(fb->b.name && fb->fp) {
		...
		free(fb->b.data);
		if(fb->b.name) {
			cli_errmsg("fileblobDestroy: %s not saved: report to http://bugs.quikav.net\n",
				(fb->fullname) ? fb->fullname : fb->b.name);
			free(fb->b.name);
		} else
			cli_errmsg("fileblobDestroy: file not saved (%lu bytes): report to http://bugs.quikav.net\n",
...
/*
 * Performs a full scan on the fileblob, returning QuikAV status:
