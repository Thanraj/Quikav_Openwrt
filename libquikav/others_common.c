...
void *cli_malloc(size_t size)
{
	...
	cli_errmsg("cli_malloc(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", (unsigned long int) size);
	return NULL;
    }
...
void *cli_calloc(size_t nmemb, size_t size)
{
	...
	cli_errmsg("cli_calloc(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", (unsigned long int) nmemb*size);
	return NULL;
    }
...
void *cli_realloc(void *ptr, size_t size)
{
    ...
    if(!size || size > CLI_MAX_ALLOCATION) {
	cli_errmsg("cli_realloc(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", (unsigned long int) size);
	return NULL;
    }
...
void *cli_realloc2(void *ptr, size_t size)
{
    ...
    if(!size || size > CLI_MAX_ALLOCATION) {
	cli_errmsg("cli_realloc2(): Attempt to allocate %lu bytes. Please report to http://bugs.quikav.net\n", (unsigned long int) size);
	return NULL;
    }
...
char *cli_strdup(const char *s)
{
    ...
    if(s == NULL) {
        cli_errmsg("cli_strdup(): s == NULL. Please report to http://bugs.quikav.net\n");
        return NULL;
    }
...
char *cli_gentemp(const char *dir)
{
	...
	snprintf(name, len, "%s"PATHSEP"quikav-%s.tmp", mdir, tmp);
    free(tmp);
