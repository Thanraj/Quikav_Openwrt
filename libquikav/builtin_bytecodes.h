...
static const char* builtin_bc_startup = "QuikBCafhhbfkjmld|afefdfggifnf```aa```|biacflfafmfbfcfmb`cnbacacmbachcccmbgfbfcc`ccchcbfdf``bgcaap`quikcoincidencejb:4096\n"
"\n"
...
#if 0
const uint16_t __quikbc_kind = BC_STARTUP;
int entrypoint()
{
  // Whole platform specific bugs can be disabled with check_platform,
  // see quikscan --debug for meaning of bits.
  // For example:
      ...
      if (env.os_category == os_linux) {
        if (env.os_features & (1 << feature_selinux))
          ...
          disable_jit_if("^SELinux is preventing 'execmem' access.\n"
                         "Run  'setsebool -P quikd_use_jit on'.", 0, 1);
          ...
          disable_jit_if("^RWX mapping denied for unknown reason."
            "Please report to http://bugs.quikav.net\n", 0, 1);
      }
    } else {
  ...
  /* magic number to tell libquikav that selftest succeeded */
  return 0xda7aba5e;
}
