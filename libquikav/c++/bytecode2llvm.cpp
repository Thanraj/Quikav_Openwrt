...
#include "QuikBCModule.h"
#include "QuikBCDiagnostics.h"
...
#error "libstdc++ parallel mode is not supported for QuikAV. Please remove -D_GLIBCXX_PARALLEL from CXXFLAGS!"
#endif
...
private:
	...
	if (operand < func->numValues) {
		...
		llvm_report_error("(libquikav) Type mismatch converting operand");
	    }
	    return V;
	}
	unsigned w = Ty->getPrimitiveSizeInBits();
	...
	// This was already validated by libquikav.
...
//	    Functions[j]->addFnAttr(Attribute::StackAlignment);
	...
	for (unsigned j=0;j<bc->num_func;j++) {
		...
		for (unsigned i=0;i<bc->num_globals;i++) {
			...
			llvm_report_error("(libquikav) unable to create fake global");
		    }
		...
		for (unsigned j=0;j<bb->numInsts && !broken;j++) {
		    ...
		    // libquikav has already validated this.
			...
			default:
				...
				case 2:
					...
					Op0->dump();
					Op1->dump();
					llvm_report_error("(libquikav) binop type mismatch");
				    }
				    break;
				case 3:
...
int cli_bytecode_prepare_jit(struct cli_all_bc *bcs)
{
  ...
  try {
    Module *M = new Module("QuikAV jit module", bcs->engine->Context);
    {
	// Create the JIT.
	std::string ErrorMsg;
#if LLVM_VERSION < 36
...
#endif
	//TODO:cli_dbgmsg
	DEBUG(errs() << "WARNING: QuikAV JIT built w/o atomic builtins\n"
	      << "On x86 for best performance QuikAV should be built for i686, not i386!\n");
    }
    return 0;
}
