...
static int ooxml_parse_document(int fd, cli_ctx *ctx)
{
    ...
    reader = xmlReaderForFd(fd, "properties.xml", NULL, QUIKAV_MIN_XMLREADER_FLAGS);
    if (reader == NULL) {
...
static int ooxml_content_cb(int fd, cli_ctx *ctx)
{
    ...
    reader = xmlReaderForFd(fd, "[Content_Types].xml", NULL, QUIKAV_MIN_XMLREADER_FLAGS);
    if (reader == NULL) {
...
static int ooxml_hwp_cb(int fd, cli_ctx *ctx)
{
    ...
    reader = xmlReaderForFd(fd, "ooxml_hwp.xml", NULL, QUIKAV_MIN_XMLREADER_FLAGS);
    if (reader == NULL) {
