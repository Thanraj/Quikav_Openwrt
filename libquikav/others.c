 ...
 *  Copyright (C) 2007-2010 Tringapps, Inc.
 *
...
#if 0
#define lt_preload_symbols lt_libquikav_LTX_preloaded_symbols
...
static lt_dlhandle lt_dlfind(const char *name, const char *featurename)
{
	...
	LT_MODULE_EXT"."LIBQUIKAV_FULLVER,
	PASTE(LT_MODULE_EXT".", LIBQUIKAV_MAJORVER),
	LT_MODULE_EXT,
	"."LT_LIBEXT
    };
...
static void cli_rarload(void) {
    ...
    rhandle = lt_dlfind("libquikunrar_iface", "unrar");
    if (!rhandle)
	return;

    if (!(cli_unrar_open = (int(*)(int, const char *, unrar_state_t *))lt_dlsym(rhandle, "libquikunrar_iface_LTX_unrar_open")) ||
	!(cli_unrar_extract_next_prepare = (int(*)(unrar_state_t *, const char *))lt_dlsym(rhandle, "libquikunrar_iface_LTX_unrar_extract_next_prepare")) ||
	!(cli_unrar_extract_next = (int(*)(unrar_state_t *, const char *))lt_dlsym(rhandle, "libquikunrar_iface_LTX_unrar_extract_next")) ||
	!(cli_unrar_close = (void(*)(unrar_state_t *))lt_dlsym(rhandle, "libquikunrar_iface_LTX_unrar_close"))
	) {
...
struct cl_engine *cl_engine_new(void)
{
    ...
    new->cb_stats_flush = quikav_stats_flush;
    new->cb_stats_remove_sample = quikav_stats_remove_sample;
    new->cb_stats_decrement_count = quikav_stats_decrement_count;
    new->cb_stats_get_num = quikav_stats_get_num;
    new->cb_stats_get_size = quikav_stats_get_size;
    new->cb_stats_get_hostid = quikav_stats_get_hostid;
...
int cli_rmdirs(const char *dirname)
{
	...
	while(QUIKSTAT(dirname, &maind) != -1) {
...
/* Implement a generic bitset, trog@quikav.net */
