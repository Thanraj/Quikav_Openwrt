 ...
 *  Copyright (C) 2010 Tringapps, Inc.
 *
...
/*
    The trick is:
    1 - Reinclude quikav-config.h which is not guarded against multiple inclusions.
	...
	This result in quik_options struct in optparser be defined with proper pointers.
*/

#include "quikav-config.h"
char _DATADIR[MAX_PATH] = DATADIR;
char _CONFDIR[MAX_PATH] = CONFDIR;
char _CONFDIR_QUIKD[MAX_PATH] = CONFDIR"\\quikd.conf";
char _CONFDIR_FRESHQUIK[MAX_PATH] = CONFDIR"\\freshquik.conf";
char _CONFDIR_MILTER[MAX_PATH] = CONFDIR"\\quikav-milter.conf";

#undef DATADIR
#undef CONFDIR
const char *DATADIR = _DATADIR;
const char *CONFDIR = _CONFDIR;
const char *CONFDIR_QUIKD = _CONFDIR_QUIKD;
const char *CONFDIR_FRESHQUIK = _CONFDIR_FRESHQUIK;
const char *CONFDIR_MILTER = _CONFDIR_MILTER;

#define DATADIR _DATADIR
#define CONFDIR _CONFDIR
#define CONFDIR_QUIKD _CONFDIR_QUIKD
#define CONFDIR_FRESHQUIK _CONFDIR_FRESHQUIK
#define CONFDIR_MILTER _CONFDIR_MILTER

#include "shared/optparser.c"

#define QUIKKEY "Software\\QuikAV"
void fix_paths(void) {
    ...
    if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, QUIKKEY, 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS || RegOpenKeyEx(HKEY_CURRENT_USER, QUIKKEY, 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS) {
	sizof = sizeof(path);
    ...
    if(have_cdir) {
	snprintf(_CONFDIR_QUIKD, sizeof(_CONFDIR_QUIKD), "%s\\%s", _CONFDIR, "quikd.conf");
	snprintf(_CONFDIR_FRESHQUIK, sizeof(_CONFDIR_FRESHQUIK), "%s\\%s", _CONFDIR, "freshquik.conf");
	snprintf(_CONFDIR_MILTER, sizeof(_CONFDIR_MILTER), "%s\\%s", _CONFDIR, "quikav-milter.conf");
    }
}
