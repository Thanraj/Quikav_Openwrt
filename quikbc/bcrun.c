...
{
    printf("\n");
    printf("           Quik AntiVirus: Bytecode Testing Tool %s\n",
	   get_version());
    printf("           By The QuikAV Team: http://www.quikav.net/about.html#credits\n");
    printf("           (C) 2009-2015 Quik AV, Inc.\n\n");
    printf("quikbc <file> [function] [param1 ...]\n\n");
...
int main(int argc, char *argv[])
{
    ...
    opts = optparse(NULL, argc, argv, 1, OPT_QUIKBC, 0, NULL);
    if (!opts) {
    ...
    if (rc != CL_SUCCESS) {
	fprintf(stderr,"Unable to init libquikav: %s\n", cl_strerror(rc));
	optfree(opts);
	exit(4);
    }
	...
	if (debug_flag)
	    printf("[quikbc] Bytecode prepared\n");
	...
	if (debug_flag)
	    printf("[quikbc] Running bytecode function :%u\n", funcid);
	...
	if (rc != CL_SUCCESS) {
	    ...
	    uint64_t v;
	    if (debug_flag)
		printf("[quikbc] Bytecode run finished\n");
	    v = cli_bytecode_context_getresult_int(ctx);
	    if (debug_flag)
		printf("[quikbc] Bytecode returned: 0x%llx\n", (long long)v);
	}
