...
static int hashsig(const struct optstruct *opts, unsigned int mdb, int type)
{
	...
	for(i = 0; opts->filename[i]; i++) {
	    if(QUIKSTAT(opts->filename[i], &sb) == -1) {
		perror("hashsig");
...
static char *getdsig(const char *host, const char *user, const unsigned char *data, unsigned int datalen, unsigned short mode)
{
	...
	perror("connect()");
        closesocket(sockd);
	mprintf("!getdsig: Can't connect to QuikAV Signing Service at %s\n", host);
	memset(pass, 0, sizeof(pass));
	return NULL;
    }
    ...
    if(mode == 1)
	snprintf(cmd, sizeof(cmd) - datalen, "QuikSign:%s:%s:", user, pass);
    else if(mode == 2)
	snprintf(cmd, sizeof(cmd) - datalen, "QuikSignPSS:%s:%s:", user, pass);
    else
	snprintf(cmd, sizeof(cmd) - datalen, "QuikSignPSS2:%s:%s:", user, pass);
...
#define FREE_LS(x)		    \
	...
	if(entries != sigs)
	    mprintf("^build: Signatures in %s db files: %u, loaded by libquikav: %u\n", dbname, entries, sigs);
...
static void help(void)
{
    ...
    mprintf("    --server=ADDR                          QuikAV Signing Service address\n");
...
int main(int argc, char **argv)
{
	...
	if(!opts->filename) {
	    ...
	    if(QUIKSTAT(opts->filename[0], &sb) == -1) {
