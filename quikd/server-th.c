...
static const char* parse_dispatch_cmd(client_conn_t *conn, struct fd_buf *buf, size_t *ppos, int *error, const struct optstruct *opts, int readtimeout)
{
	...
	if (*error || !conn->group || rc) {
		 ...
		 * and closed the connection, so quikd closes the
...
int recvloop_th(int *socketds, unsigned nsockets, struct cl_engine *engine, unsigned int dboptions, const struct optstruct *opts)
{
...
#if defined(FANOTIFY) || defined(QUIKAUTH)
	pthread_t fan_pid;
    ...
    /* options are handled in main (quikd.c) */
	...
	const unsigned quikdfiles = 6;
#ifdef C_SOLARIS
	int solaris_has_extended_stdio = 0;
#endif
	/* Condition to not run out of file descriptors:
	 * MaxThreads * MaxRecursion + (MaxQueue - MaxThreads) + QUIKDFILES < RLIMIT_NOFILE 
	 * QUIKDFILES is 6: 3 standard FD + logfile + 2 FD for reloading the DB
	 * */
#ifdef C_SOLARIS
	...
	*/
	...
	if (enable_extended_FILE_stdio(-1, -1) == -1) {
	    logg("^Unable to set extended FILE stdio, quikd will be limited to max 256 open files\n");
	...
	*/
	...
	maxrec = opt->numarg;
	max_max_queue = rlim.rlim_cur - maxrec * max_threads - quikdfiles + max_threads;
