...
int command(client_conn_t *conn, int *virus)
{
	...
	case COMMAND_MULTISCAN: {
	    ...
	    if (QUIKSTAT(conn->filename, &sb) == 0 &&
		!S_ISDIR(sb.st_mode)) {
		thrmgr_setactivetask(NULL, "CONTSCAN");
		type = TYPE_CONTSCAN;
		break;
	...
	case COMMAND_FILDES:
	 ...
	 case COMMAND_ALLMATCHSCAN:
		...
		conn_reply(conn, conn->filename, "ALLMATCHSCAN command disabled by quikd configuration.", "ERROR");
		return 1;
	    }
...
static int print_ver(int desc, char term, const struct cl_engine *engine)
{
	...
	return mdprintf(desc, "QuikAV %s/%u/%s%c", get_version(), (unsigned int) ver, tstr, term);
    }
    return mdprintf(desc, "QuikAV %s%c", get_version(), term);
}
...
static void print_commands(int desc, char term, const struct cl_engine *engine)
{
    ...
    const char *quikd_ver = get_version();
    if (strcmp(engine_ver, quikd_ver)) {
	mdprintf(desc, "ENGINE VERSION MISMATCH: %s != %s. ERROR%c",
		 engine_ver, quikd_ver, term);
	return;
    }
