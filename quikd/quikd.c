...
int main(int argc, char **argv)
{
    ...
    if((opts = optparse(NULL, argc, argv, 1, OPT_QUIKD, 0, NULL)) == NULL) {
    ...
    if((opts = optparse(cfgfile, 0, NULL, 1, OPT_QUIKD, 0, opts)) == NULL) {
    ...
    do { /* logger initialized */
        ...
        logg("#quikd daemon %s (OS: "TARGET_OS_TYPE", ARCH: "TARGET_ARCH_TYPE", CPU: "TARGET_CPU_TYPE")\n", get_version());
        ...
        cl_engine_set_clcb_virus_found(engine, quikd_virus_found_cb);
        ...
        if(localsock && num_fd == 0) {
            ...
            if(optget(opts, "LocalSocketMode")->enabled) {
                ...
                sock_mode = 0777 /* & ~umsk*/; /* conservative default: umask was 0 in quikd < 0.96 */
            }
        ...
        {
#ifdef C_BSD	    
            /* workaround for OpenBSD bug, see https://wwws.quikav.net/bugzilla/show_bug.cgi?id=885 */
