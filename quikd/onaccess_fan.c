 ...
 *  Copyright (C) 2011 Tringapps, Inc.
 *
...
void *onas_fan_th(void *arg)
{
	...
	if(errno == EPERM)
	    logg("ScanOnAccess: quikd must be started by root\n");
	return NULL;
    }
...
void *onas_fan_th(void *arg)
{
    ...
    cauth_fd = open("/dev/quikauth", O_RDONLY);
    if(cauth_fd == -1) {
	logg("!ScanOnAccess: Can't open /dev/quikauth\n");
	if(errno == ENOENT)
	    logg("!ScanOnAccess: Please make sure QuikAuth.kext is loaded\n");
	else if(errno == EACCES)
	    logg("!ScanOnAccess: This application requires root privileges\n");
	else
	    logg("!ScanOnAccess: /dev/quikauth: %s\n", cli_strerror(errno, err, sizeof(err)));
	...
	if(read(cauth_fd, &event, sizeof(event)) > 0) {
		...
		if(strncmp(event.path, "QuikAuth", 8)) {
	    ...
	    if(errno == ENODEV) {
		printf("^ScanOnAccess: QuikAuth module deactivated, terminating\n");
		close(cauth_fd);
		return NULL;
	    }
	}
