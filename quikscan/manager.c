 ...
 *  Copyright (C) 2007-2013 Tringapps, Inc.
 *
 *  Authors: Tomasz Kojm
 *
...
#include "libquikav/matcher-ac.h"
#include "libquikav/matcher-pcre.h"
#include "libquikav/str.h"
#include "libquikav/readdb.h"
...
static cl_error_t pre(int fd, const char *type, void *context)
{
    ...
    if (!(context))
        return CL_CLEAN;
    d = (struct quikscan_cb_data *)context;
    c = d->chain;
    if (c == NULL)
        return CL_CLEAN;
...
{
    struct quikscan_cb_data *d = context;
    struct metachain *c;
    char str[128];
...
static void quikscan_virus_found_cb(int fd, const char *virname, void *context)
{
    struct quikscan_cb_data *data = (struct quikscan_cb_data *)context;
    const char * filename;
...
static int scanstdin(const struct cl_engine *engine, const struct optstruct *opts, int options)
{
    ...
    if(checkaccess(tmpdir, QUIKAVUSER, W_OK) != 1) {
...
int scanmanager(const struct optstruct *opts)
{
    ...
    if((ret = cl_init(CL_INIT_DEFAULT))) {
        logg("!Can't initialize libquikav: %s\n", cl_strerror(ret));
        return 2;
    }
    ...
    if(optget(opts, "allmatch")->enabled) {
        ...
        cl_engine_set_clcb_virus_found(engine, quikscan_virus_found_cb);
    }
...
#ifdef C_LINUX
    procdev = (dev_t) 0;
    if(QUIKSTAT("/proc", &sb) != -1 && !sb.st_size)
        procdev = sb.st_dev;
#endif
